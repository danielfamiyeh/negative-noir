(function() {
    var canvas = document.getElementById('canvas'),
        ...
        playColour = '#000000',
        muteColour = '#000000',
        firstTime = localStorage.getItem('firstTimee') || true,
        fontSize, lastTwenty = 0,
        multi,
        filled = false,
        adService, leaderboard, isOnline = false,
        banner, status, bgColourList = ['#87ceeb', '#8F9BFE', '#DD5F4D', '#EB899E', '#9eeb89'],
        loaded, practiceText, loggedIn, maxNum = 1,
        colIndex = 0,
        social, practiceLeft = 10,
        scoreText, socialService, spawnCount = 985,
        count = 0,
        text, permaHighScore, permaHardHighScore, speed, started = false,
        first = localStorage.getItem('first') || false,
        second = localStorage.getItem('second') || false,
        third = localStorage.getItem('third') || false,
        fourth = localStorage.getItem('fourth') || false,
        fifth = localStorage.getItem('fifth') || false,
        sixth = localStorage.getItem('sixth') || false,
        seventh = localStorage.getItem('seventh') || false,
        eighth = localStorage.getItem('eighth') || false,
        hallOfFame = false,
        loggedIn = false,
        score = 0,
        str = score.toString(),
        hardHighScoree = localStorage.getItem('hardHighScoree') || 0,
        WIDTH = window.innerWidth,
        HEIGHT = window.innerHeight;
        
    canvas.width = WIDTH;
    canvas.height = HEIGHT;
    speed = ((WIDTH * HEIGHT) / 382010.666667);
    ...
    function AABBCollision(shapeA, shapeB) {
        if (shapeA.x + shapeA.width > shapeB.x && shapeA.x < shapeB.x + shapeB.width && shapeA.y + shapeA.height > shapeB.y && shapeA.y < shapeB.y + shapeB.height) {
            return true;
        }
    };

    function getFont(fontSize) {
        var ratio = fontSize / (984);
        var size = WIDTH * ratio;
        return (size | 0) + 'px lato';
    }
    ...
    function touchHandler(event) {
        ...
        if (gameState == 'start' || gameState == 'dead') {
            if (touchX > 356 && touchY > 40 &&
                touchX < 646 && touchY < 120) {
        ...
        if (gameState === 'start') {
            if (touchX > (WIDTH / 8.41025641) && touchY > (HEIGHT / 2.52456647) && touchX < (WIDTH - WIDTH / 1.75) && touchY < (HEIGHT / 1.65592417)) {
                gameState = 'help';
            }
            if (touchX > (WIDTH / 1.80882353) && touchY > (HEIGHT / 2.54664726) && touchX < (WIDTH / 1.11818182) && touchY < (HEIGHT / 1.59981685)) {
                if (!socialService.isLoggedIn()) {
                    alert('Must be signed into ' + multi);
                } else {
            ...
            if (touchX > (WIDTH / 8) && touchY > (HEIGHT / 1.36912226) && touchX < (WIDTH / 1.03114187) && touchY < (HEIGHT / 1.28834808)) {
                if (!socialService.isLoggedIn()) {
                    alert('Must be signed into ' + multi);
                } else {
        ...
        if (gameState == 'start' || gameState == 'dead') {
            if (touchX > (WIDTH / 2.48484848) && touchX < (WIDTH / 1.62376238) && touchY > (HEIGHT / 1.153125) && touchY < (HEIGHT / 1.10810811)) {
            ...
            if (touchX > ((WIDTH / 2) / 1.394) && touchX < ((WIDTH / 2) / 0.748) && touchY > ((HEIGHT / 2) + ((HEIGHT / 2) / 2.09975962)) && touchY < (HEIGHT / 2) / 0.617) {
                enemyList = [];
                player.bullets = [];
                gameState = 'play';
            }
        ...
        if (gameState == 'play' || gameState == 'help') {
            if (touchX > player.rect.x && touchX < player.rect.x + player.rect.width && touchY > player.rect.y && touchY < player.rect.y + player.rect.height) {
                if (muteColour == '#000000') {
    ...
    function render() {
        ...
        if (gameState === 'start') {
            ...
            ctx.lineTo(WIDTH - WIDTH / 1.75, HEIGHT / 2);
        ...
        if (gameState === 'dead') {
            ...
            if (score > permaHardHighScore) {
                ...
                ctx.font = getFont(64);
                ctx.fillText('You beat your high score.', (WIDTH / 2), (HEIGHT / 3.86504425));
            };
            ctx.font = getFont(64);
    ...
    function update() {
        ...
        if (gameState == 'play' || gameState == 'help') {
            for (var i = 0; i < player.bullets.length; i++) {
                ...
                if (player.bullets[i].x < 0 || player.bullets[i].x > WIDTH || player.bullets[i].y < 0 || player.bullets[i].y > HEIGHT) {
                    player.bullets.splice(i, 1);
                };
            };
        }
                    ...
                    if (AABBCollision(player.bullets[i].rect, enemyList[j])) {
                        if (player.bullets[i].isActive && enemyList[j].isActive) {
                            if (player.bullets[i].colour != enemyList[j].colour) {
                                ...
                                count += 1;
                                score += 1;
                                if (score % 20 == 0 && score != lastTwenty) {
                                    lastTwenty = score;
        ...
        for (var i = 0; i < enemyList.length; i++) {
            if (AABBCollision(player.rect, enemyList[i])) {
                ...
                spawnCount = 985;
                count = 0;
                permaHardHighScore = hardHighScoree;
                ...
                if (score >= 300 && !fifth) {
                    socialService.submitAchievement('CgkImrjO8Y8JEAIQBg', function(error) {
                        if (error)
                            console.error("showLeaderbord error: " + error.message);
                    });
                ...
                if (score >= 500 && !sixth) {
                    socialService.submitAchievement('CgkImrjO8Y8JEAIQBw', function(error) {
                        if (error)
                            console.error("showLeaderbord error: " + error.message);
                    });
                    localStorage.setItem('sixth', true);
                }

                if (score >= 750 && !seventh) {
                    socialService.submitAchievement('CgkImrjO8Y8JEAIQCA', function(error) {
                        if (error)
                            console.error("showLeaderbord error: " + error.message);
                    });
                    localStorage.setItem('seventh', true);
                }

                if (score >= 1000 && !eighth) {
                    socialService.submitAchievement('CgkImrjO8Y8JEAIQCQ', function(error) {
                        if (error)
                            console.error("showLeaderbord error: " + error.message);
                    });
                    localStorage.setItem('eighth', true);
                }
        ...
        if (gameState == 'dead') {
            if (score > hardHighScoree) {
                hardHighScoree = parseInt(score);
                localStorage.setItem('hardHighScore', hardHighScoree);
                socialService.submitScore(hardHighScoree, function(error) {
                    if (error)
    ...
    function init() {
        ...
        adService.configure({
            ios: {
                banner: "ca-app-pub-1975599370935900/8913107473",
                interstitial: "ca-app-pub-1975599370935900/1389840670"
            },
            android: {
                banner: "ca-app-pub-1975599370935900/4482907877",
                interstitial: "ca-app-pub-1975599370935900/5959641072"
            }
        });
